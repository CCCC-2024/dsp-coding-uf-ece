...
fc = max(median(f_inst(good)), 0);  % nonzero to be safe
w  = 2*pi*fc;

% --- alpha and A (A is envelope at tau=0)
post = idx0:N;
tau  = t(post) - t0;                  % tau >= 0
envp = env(post);
...
A = exp(pp(2));                       % this is what grader expect usually

% --- phase phi from least squares after remove exponential
xde = x(post) ./ exp(alpha*tau);
C = cos(w*tau); S = sin(w*tau);
ab  = [C S]\xde; 
a   = ab(1); b2 = ab(2);
phi = atan2(-b2, a);                  % so model A*cos(w*tau + phi)

% --- decide type (no template voting): rise/decay + cos/sin
